...
def y_grammar(p): return postr(passing,M(y_entry))(p)
...
def y_seq_object(p): return P(postr(pzero,S(y_term,s("*"))),postr(pmore,S(y_term,s("+"))),postr(poptional,S(y_term,s("?"))),postr(passing,y_term),postr(passing,y_symbol),postr(poptional,S(y_keyword,s("?"))),postr(passing,y_keyword))(p)
def y_term(p): return postr(passing,y_identifier)(p)
def y_processing(p): return postr(passing,y_identifier)(p)
